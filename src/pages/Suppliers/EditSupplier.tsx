...
export function EditSupplier() {
  ...
  const [saving, setSaving] = useState(false); // Loading for saving changes
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      ...
      try {
        setLoading(true);
        const supplierData = await databaseService.suppliers.getById(id);
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!id) return; // Should not happen if accessed via route with ID
    ...
    try {
      ...
      const updatedSupplier: Partial<Supplier> = {
        ...
        // Supabase will handle updated_at
      };
  ...
  if (loading) {
    return <div className="text-center text-gray-600">Carregando dados do fornecedor...</div>;
  }
        ...
        <div className="relative px-4 py-10 bg-white shadow-lg sm:rounded-3xl sm:p-20">
          <div className="max-w-md mx-auto">
            <div className="flex items-center justify-center">
              ...
              <h2 className="text-3xl font-extrabold text-gray-900 ml-3">Editar Fornecedor</h2>
            </div>
            <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <button
                  ...
                  disabled={saving}
                >
                  {saving ? 'Salvando...' : 'Atualizar Fornecedor'}
                </button>
              </div>
            </form>
          </div>
